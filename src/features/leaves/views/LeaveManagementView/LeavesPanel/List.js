...
const List = ({
  ...
  const canApprove = can(PERMISSIONS.LEAVES_APPROVE);
  const [selectedIds, setSelectedIds] = React.useState([]);

  const selectionActions = canApprove
    ? [
        {
          label: "Approve selected",
          icon: <ThumbUpIcon />,
          handler: () => onBulkApprove?.(selectedIds),
        },
        {
          label: "Reject selected",
          icon: <ThumbDownIcon />,
          handler: () => onBulkReject?.(selectedIds),
        },
      ]
    : [];
  ...
  return (
    <>
      <TableComponent
        size="small"
        toolbarSelectionTitle="leave requests"
        ...
        columns={[
          {
            ...
            renderCell: ({ employeeId }) => {
              ...
              return <Typography variant="h6">{name || "Unknown"}</Typography>;
            },
          },
          {
            field: "leaveType",
            ...
            renderCell: ({ leaveType }) => {
              const lt = leaveTypeMap[leaveType] || leaveTypeMap[String(leaveType).toLowerCase()];
              const label = lt?.name || lt?.title || leaveType;
              const color = lt?.color || "#2196f3";
              return (
                <Chip
                  size="small"
            ...
            ? [
                {
                  ...
                  hide: ({ status }) => status === "approved",
                },
                {
                  label: "Reject Leave",
                  icon: <ThumbDownIcon />,
                  handler: ({ _id }) => onRejectLeaveClicked?.(_id),
                  hide: ({ status }) => status === "rejected",
                },
              ]
            : []),
          {
