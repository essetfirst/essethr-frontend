...
import { employeeNameFromMap, leaveTypeLabel } from "utils/employeeDisplay";
...
const LeavesPanel = ({
  ...
  const employeesMap = React.useMemo(() => {
    const fromOrg = arrayToMap(org?.employees || [], "_id");
    (state.fetchLeaves.leaves || []).forEach((l) => {
      if (l.employee && l.employeeId) {
        fromOrg[String(l.employeeId)] = l.employee;
      }
    });
    return fromOrg;
  }, [org?.employees, state.fetchLeaves.leaves]);

  const leaveTypeMap = React.useMemo(() => {
    const map = {};
    (org.leaveTypes || []).forEach((lt) => {
      map[String(lt._id)] = lt;
      if (lt.title) map[String(lt.title).toLowerCase()] = lt;
    });
    return map;
  }, [org.leaveTypes]);

  const leaveTypes = [
    ...(org.leaveTypes || []).map(({ _id, name, title }) => ({
      label: name || title,
      value: _id,
    })),
  ];
  ...
  const handleExportClick = async () => {
    ...
    const rows = state.fetchLeaves.leaves.map((leave) => {
      ...
      return {
        employeeId: employeeNameFromMap(employeesMap, employeeId),
        leaveType: leaveTypeLabel(leaveType, leaveTypeMap),
        duration,
        startDate,
        endDate,
        comment,
  ...
  const handleRejectLeaveClick = (selected) => {
    onRejectLeaves([selected]);
  };

  const handleBulkApprove = (ids) => {
    if (ids?.length) onApproveLeaves(ids);
  };

  const handleBulkReject = (ids) => {
    if (ids?.length) onRejectLeaves(ids);
  };
  ...
  const employeesOnLeave = (state.fetchLeaves.leaves || [])
    .filter(
      (l) =>
        l.status === "approved" &&
        periodIncludesToday(
    ...
    .map((l) => {
      const emp = employeesMap[String(l.employeeId)] || {};
      return {
        employee: {
          name: employeeNameFromMap(employeesMap, l.employeeId),
          image: emp.image,
        },
        leaveType: leaveTypeLabel(l.leaveType, leaveTypeMap),
        duration: l.duration,
        to: l.endDate || l.to,
      };
    });
  ...
  return (
    <div>
      ...
      <List
        ...
        onRejectLeaveClicked={handleRejectLeaveClick}
