import { fmt, fmtNow, monthBounds } from "utils/date";
import React from "react";
import { Box, Chip, TextField, Typography } from "@mui/material";
import { Payment as PayrollHoursIcon } from "@mui/icons-material";
  ...
  return (
    <Box display="flex" alignItems={"center"} justifyContent={"space-between"}>
      <TextField
        ...
        value={
          filters.fromDate || monthBounds().start
        }
      ...
      <TextField
        ...
        name="to"
        onChange={onFilterFieldChange}
        value={filters.toDate || monthBounds().end}
        style={{ marginRight: "8px" }}
      />
    </Box>
  );
...
const PayrollHoursView = () => {
  ...
  const handleExportClick = async () => {
    if (state.payrollHours.length > 0) {
      ...
      const filename = `Payroll Hours ${fmtNow("dd-MM-yyyy")}`;
